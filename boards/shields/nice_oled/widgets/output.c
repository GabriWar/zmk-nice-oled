...
static void draw_usb_connected(lv_obj_t *canvas) {
    ...
    canvas_draw_img(canvas, CONFIG_NICE_OLED_WIDGET_OUTPUT_USB_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_OUTPUT_USB_CUSTOM_Y, &usb, &img_dsc);
}
...
static void draw_ble_unbonded(lv_obj_t *canvas) {
    ...
    canvas_draw_img(canvas, CONFIG_NICE_OLED_WIDGET_OUTPUT_BT_UNBONDED_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_OUTPUT_BT_UNBONDED_CUSTOM_Y, &bt_unbonded, &img_dsc);
}
#endif
...
static void draw_ble_disconnected(lv_obj_t *canvas) {
    ...
    canvas_draw_img(canvas, CONFIG_NICE_OLED_WIDGET_OUTPUT_BT_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_OUTPUT_BT_CUSTOM_Y, &bt_no_signal, &img_dsc);
}
...
static void draw_ble_connected(lv_obj_t *canvas) {
    ...
    canvas_draw_img(canvas, CONFIG_NICE_OLED_WIDGET_OUTPUT_BT_CUSTOM_X, CONFIG_NICE_OLED_WIDGET_OUTPUT_BT_CUSTOM_Y, &bt, &img_dsc);
}
...
void draw_output_status(lv_obj_t *canvas, const struct status_state *state) {
    ...
    lv_draw_rect_dsc_t rect_white_dsc;
    init_rect_dsc(&rect_white_dsc, LVGL_FOREGROUND);
    canvas_draw_rect(canvas, -3, 32, 24, 15, &rect_white_dsc);
#endif
